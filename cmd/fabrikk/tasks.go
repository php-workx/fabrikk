...
// cmdTasks implements `fabrikk tasks` (spec section 5.2).
func cmdTasks(args []string) error {
	runID, f := parseTaskFilter(args)
	if runID == "" {
		return fmt.Errorf("usage: fabrikk tasks <run-id> [--status X] [--task-type X] [--tag X] [--json]")
	}
...
// cmdReady implements `fabrikk ready` (spec section 5.2).
func cmdReady(args []string) error {
	runID, f := parseTaskFilter(args)
	if runID == "" {
		return fmt.Errorf("usage: fabrikk ready <run-id> [--json]")
	}
...
// cmdBlocked implements `fabrikk blocked` (spec section 5.2).
func cmdBlocked(args []string) error {
	runID, f := parseTaskFilter(args)
	if runID == "" {
		return fmt.Errorf("usage: fabrikk blocked <run-id> [--json]")
	}
...
func cmdNext(args []string) error {
	runID, f := parseTaskFilter(args)
	if runID == "" {
		return fmt.Errorf("usage: fabrikk next <run-id> [--json]")
	}
...
// cmdProgress implements `fabrikk progress` (spec section 5.2).
func cmdProgress(args []string) error {
	runID, f := parseTaskFilter(args)
	if runID == "" {
		return fmt.Errorf("usage: fabrikk progress <run-id> [--json]")
	}
