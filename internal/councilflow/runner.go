...
func runParallelReviews(ctx context.Context, r *Runner, spec string, round int, pending []pendingReview, priorFindings []ReviewOutput, codebaseCtx string) []ReviewOutput {
	...
	for i, p := range pending {
		...
		go func(pr pendingReview) {
			backend := agentcli.BackendFor(pr.persona.Backend, pr.persona.ModelPref)
...
func (r *Runner) runSingleReview(ctx context.Context, spec string, round int, persona *Persona, backend *agentcli.CLIBackend, priorFindings []ReviewOutput, codebaseCtx string) (*ReviewOutput, error) {
	...
	// Initial review.
	output, err := agentcli.InvokeFunc(ctx, backend, prompt, r.TimeoutSec)
	if err != nil {
	...
	nudgeOutput, nudgeErr := agentcli.InvokeFunc(ctx, backend, nudgePrompt, r.TimeoutSec)
	if nudgeErr != nil {
...
func extractJSON(s string) string {
	...
	if candidate := agentcli.ExtractFromCodeFence(s); candidate != "" {
	...
	if idx >= 0 {
		if result := agentcli.ExtractJSONBlock(s, idx); result != "" {
			return result
		}
	}
